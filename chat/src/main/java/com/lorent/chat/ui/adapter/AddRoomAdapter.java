package com.lorent.chat.ui.adapter;

import android.content.Context;
import android.view.View;
import android.view.ViewGroup;
import android.widget.BaseAdapter;
import android.widget.TextView;

import com.lorent.chat.R;
import com.lorent.chat.ui.entity.GroupChatService;
import com.lorent.chat.ui.view.CircleImageView;

import java.util.List;

public class AddRoomAdapter extends BaseAdapter {
	private Context mContext;
	private List<GroupChatService> lists;

	public AddRoomAdapter(Context context, List<GroupChatService> lists) {
		this.mContext = context;
		this.lists = lists;
	}

	@Override
	public int getCount() {
		if (lists != null) {
			return lists.size();
		}
		return 0;
	}

	@Override
	public Object getItem(int position) {
		return lists.get(position);
	}

	@Override
	public long getItemId(int position) {
		return position;
	}

	@Override
	public View getView(int position, View convertView, ViewGroup parent) {
		final Holder holder;
		if (convertView == null) {
			convertView = View.inflate(mContext, R.layout.fragment_room_item,null);
			holder = new Holder();
			holder.iv = (CircleImageView) convertView.findViewById(R.id.user_head);
			holder.tv_name = (TextView) convertView.findViewById(R.id.name);
			holder.tv_id = (TextView) convertView.findViewById(R.id.account);
			holder.tv_tips = (TextView) convertView.findViewById(R.id.tips);
			holder.tv_content = (TextView) convertView.findViewById(R.id.content);
			convertView.setTag(holder);
		} else {
			holder = (Holder) convertView.getTag();
		}
		
		GroupChatService user = lists.get(position);
		holder.tv_id.setText(user.getGcsJID());
		holder.tv_name.setText(user.getGcsName());
		return convertView;
	}

	class Holder {
		CircleImageView iv;
		TextView tv_id;
		TextView tv_name;
		TextView tv_tips;
		TextView tv_content;
	}

}
