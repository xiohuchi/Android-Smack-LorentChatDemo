...
public abstract class ImageWorker {
     ...
     * @param imageView
     */
    public void loadLocalImage(String uri,ImageView imageView){
    	 if (uri == null) {
             return;
         }
         Bitmap bitmap = null;
         if (mImageCache != null) {
             bitmap = mImageCache.getBitmapFromMemCache(String.valueOf(uri));
         }
         if (bitmap != null) {
             imageView.setImageBitmap(bitmap);
         } else{
        	 try {
				bitmap = ImageResizer.decodeSampledBitmapFromFile(uri, 100, 200);
        ...
        if (mFadeInBitmap) {
            ...
            final TransitionDrawable td =
                    new TransitionDrawable(new Drawable[] {
                    		mLoadingBitmap != null ? getLoadingDrawable() :
